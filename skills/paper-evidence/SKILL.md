...
首次有效检索后，本 Run 的 Need ID 集合即被冻结。后续只能保留原 `id/objective` 补检索未解决 Need；不得用新 ID 重述原需求或借此绕过停止信号。如果误建新 ID，工具会给一次改回原 ID 的纠正机会，连续违反才停止。因此首次调用仍应覆盖回答所需的全部独立事实。
...
- 单个 Need 最多执行首次检索和三次有效补检索；达到读取预算后会进入 `need_stopped / answer`，应仅用已读证据回答，不再换词扫描整篇论文。
...
论文相关回答必须通过 `submit_answer` 完成。无论来源是在本轮读取还是由当前论文版本的历史证据上下文复用，依赖论文的每个事实性答案块都只绑定实际读取且确实支持该块的 `sourceObjectIds`；不要添加自编号引用或引用未读取的来源。本轮已经取得 FIGURE 视觉来源时，使用图像形成的答案块必须绑定相应 FIGURE 来源，不能只绑定讨论该图的正文。一般知识回答也使用 `submit_answer`，此时 `sourceObjectIds` 为空。若结论是跨来源推断，应明确使用“根据这些结果可以推断”等措辞，并绑定构成推断的来源。论文没有提供足够证据时，缩小回答范围；无法确认的细节不要写入答案，也不要输出检索限制或内部诊断段落。
