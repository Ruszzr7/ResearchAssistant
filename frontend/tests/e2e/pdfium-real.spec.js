...
test('PDFium preserves real two-column math-dense text facts', async ({ page }) => {
  ...
  expect(result.pageThreeReadableHasCoefficient).toBe(true)
  expect(result.pageThreeReadableHasLineBreak).toBe(false)
  expect(result.pageThreeReadableHasIllegalCharacter).toBe(false)
  expect(result.pageThreePreviewWidth).toBeGreaterThan(200)
  expect(result.pageThreePreviewHeight).toBeGreaterThan(50)
  expect(result.pageThreeDenseMinimumX).toBeGreaterThan(0.5)
  expect(result.pageThreeHitDelta).toBeLessThanOrEqual(10)
  expect(result.pageThreePayloadSegmentCount).toBeLessThanOrEqual(100)
  expect(result.pageThreePayloadMaxTextLength).toBeLessThanOrEqual(1200)
  expect(result.pageThreePayloadEndsAtSelection).toBe(true)
})
