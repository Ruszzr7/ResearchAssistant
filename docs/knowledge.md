...
- 二维公式不能继续借用 PDF.js 文字层的线性选区语义。前端提交页码和归一化矩形，并可复用当前 Canvas 提供低延迟候选裁图；服务端原 PDF 重渲染仍是 Canvas 不可用时的权威兜底。版面制品中的 `STRUCTURED` LaTeX 可直接复用，多模态/OCR 输出只能是可编辑候选。只有用户确认后的公式才以 `paperId + documentHash + parserVersion + regionKey` 独立持久化，并生成 `SelectionAnchor(FORMULA)` 与稳定 evidence；候选、低置信度区域和客户端截图均不能进入 Evidence Gate。
...
- 选区问答在产品上可以呈现为连续对话，但每一轮仍应是独立、可恢复、可审计的 workbench run。客户端只传稳定 `conversationId`，不得上传 `conversationContext`；后端从已通过门禁的服务端轮次中按预算恢复历史。全文召回查询组合当前问题、当轮固定选区和最近追问，历史只用于理解追问关系、不是论文证据。拖选草稿不能改变会话或当前固定上下文；确认新选区后沿用会话但每轮保存自己的锚点，只有“新对话”更换会话 ID。
...
- 论文精读的“捕获内容”和“开始对话”应分成两个状态：拖选文字或框选公式只产生可修改草稿，用户显式确认后才冻结锚点并启用输入框。公式的确认同时是证据准入，文字的确认至少是前端会话准入；切换草稿不能清空旧对话，确认后只替换下一轮使用的固定上下文。内容/公式切换属于右侧助手状态，但实际鼠标工具仍由唯一的 PDF 阅读器管理，不能再在顶栏维护第二套公式按钮。
...
- 选区问答的冻结上下文不能只绑定 run、论文版本和问题；重试前的 canonical anchor 仍可能因解析结果或持久化输入变化而不同。快照身份还必须覆盖页码、选区类型、原文、block、字符范围与归一化坐标，任一变化都应重建上下文。
- 对话历史、论文画像和长期观察只能扩展检索词或帮助理解代词，不能晋升为当轮事实证据。模型输入应单列当前 evidence 的 PDF hash、解析器版本、block、页码与坐标；引用门禁只接受本轮清单内的 evidenceId，来源冲突时始终以当前 PDF 版本证据为准。
- 上下文预算必须同时约束最终渲染字符串与检索扩展字符串。当前问题优先级最高，执行层应先为完整合法问题预留空间，再分配辅助上下文；历史和画像不能通过一个超长字段挤掉选区或绕过总长度限制。
- PDF 搜索的“零命中”只有在所有可搜索页面均完成索引时才是确定结果。只要存在提取失败页，就必须显示“不完整索引”并提供重试；全 PDF 无文字层、全部提取失败、部分失败和正常无命中是四种不同状态。
- `SelectionAnchor(REGION)` 是可恢复的低可信降级，不等同于精确文本锚点。界面应明确说明仅能页面定位并建议重选；若用户仍继续，模型和引用层只能要求回到原页核对，不能把区域提示伪装成已解析正文。
- PDF 选区的“能否精确映射”和“能否支撑论文结论”是两个独立维度。参考文献、页眉等内容可以有 `EXACT` 字符映射，但证据用途只能是 `METADATA_ONLY` 或 `VISUAL_ONLY`；正文也可能只有 `PARTIAL/REGION` 映射。前端降级提示、Workflow scope 和 Evidence Gate 应分别读取映射状态与证据用途，不能再用单一 `kind=REGION` 同时表达两件事。
- PDF 原文比对要先做 Unicode NFKC 归一化，并忽略软连字符、排版连字符、空白与标点，同时保留归一化字符到原始 UTF-16 偏移的映射。这样可处理 `ﬁ` 等连字和跨行断词，并把匹配结果还原成一个或多个 canonical block 字符范围；归一化文本本身不能替代原文存储。
...
- PDF 引擎派生的数学片段是可丢弃的辅助证据，不是选区锚点成立的前置条件。空白、缺字或无法映射的片段可以降低数学理解质量，但不能在页码、字符范围和几何仍有效时阻断用户固定选区。
- PDF 引擎的视觉行字符流不应直接作为用户可读文本。字符锚点保留原始字符范围，界面与语义请求使用单独派生的 NFC 可读文本；确定性地移除非法控制/未映射字符并折叠视觉换行，不在这一层猜测缺失字形、断词或二维数学结构。
- 当可选文字层包含数学但无法证明字符映射完整时，从已渲染页面按可信选区几何生成一张本地原图，比逐字猜测字体编码更可靠。该图只作当前查看和模型视觉降级，不进入持久化锚点；裁剪失败不得影响正文选区成立。
- 数学选区的模型视觉上下文应在执行时由服务端根据权威 PDF、文档指纹和规范化坐标重新生成，不能信任或持久化浏览器 data URL。图像能力是可降级增强：现有模型拒绝图像或本地裁图失败时继续使用已清洗文本，并向模型和用户明确标记公式信息不完整，禁止静默猜测。
- 数学公式含有高低不一的根号、求和上下限和上下标时，单个总包围盒会不可避免地覆盖相邻行；不能通过压缩垂直边界修复，否则会切掉合法公式结构。应保留可信字符范围，并用实际选中字形矩形作为图像可见遮罩，只给抗锯齿留极小像素边缘。
- 公式框选的浏览器 Canvas 裁图可以作为低延迟的多模态候选输入，但必须限制 MIME、编码大小和解码像素，并在服务端裁白边与降采样。客户端图片不能直接建立证据；用户确认的 LaTeX 仍需绑定当前 PDF hash、解析版本和规范区域。Canvas 不可用时保留服务端原 PDF 裁剪兜底。
- Vue `KeepAlive` 恢复 PDF 阅读器时不能只恢复事件监听和容器宽度。停用必须取消页渲染，激活后等待非零布局并重绘可见页；Canvas 像素成功与 PDFium 交互几何成功应分开记账，后者失败不能抹掉前者。文档加载使用 generation 隔离快速切换产生的旧异步结果。
- 模型回答不能由正则拼接 HTML。对话和档案应共享成熟 Markdown 解析器，关闭原始 HTML，限制链接与 KaTeX trust，并同时支持 GFM 表格、代码围栏、美元和括号数学分隔符；格式错误时必须保留可读原文。claims/evidence 是可核对出处而非思考过程，默认折叠但不能删除页码回链。
- 当前共享回答渲染器复用 MIT 许可的 Markdown-It、`@mdit/plugin-tex` 与项目既有 KaTeX，依赖随 Vite 前端打包，不需要外部服务。数学插件锁在支持 Node 18 的 0.x 版本范围，避免无意把部署门槛提升到 Node 22，也避免打包第二份 KaTeX。
...
- 公式图片转 LaTeX 应是用户主动触发的精度后手，而不是框选完成事件的副作用。调用顺序必须先查当前 PDF 版本的精确确认缓存、同页高 IoU 确认缓存和 STRUCTURED 版面公式，再进行 PDF 渲染与模型调用。相似区域只能复用人工/版面已确认结果，不能传播未确认候选；模型只返回紧凑 LaTeX JSON，`LENGTH`、坏 JSON 或低置信度直接进入可编辑区域，不自动重试。页面栅格可以按 PDF hash 做小容量缓存，但确认公式命中应发生在渲染之前。
- PDFium 内容片段是对字符锚点的有界辅助描述，不能用 `slice(0, N)` 直接截断：数学密集段落会产生大量字体 run，截断会让范围尾部与片段证据矛盾。客户端应先清理非法控制字符、拆分超长文本，再把过多片段按原顺序压缩到接口上限，同时保留首尾字符范围和数学类型；服务端在文档版本、页码、字符范围及非空原文均有效时，应承认 PDFium 原生选区的精确定位事实，不要求 PDFBox 再次逐字复现专用数学字体。
- 强推理模型执行论文摘要、JSON 修复或图片转 LaTeX 等确定性提取任务时，提示词中的“不要推理”不等于关闭 provider 的隐藏 thinking；隐藏推理仍会计入 completion token，并可能在最终 JSON 前触发 `LENGTH`。只有 provider 明确支持且经真实请求验证时，才在结构化任务的兼容适配层关闭 thinking；普通问答保持模型默认推理，其他 provider 不发送专有字段。LangChain4j 1.15.1 的 unknown custom parameter 会嵌套序列化，不能假设它等同于顶层 OpenAI 兼容字段；原生薄适配必须继续复用统一设置、预算、超时、token 统计和安全日志。
- 运行时 YAML 可以覆盖 Java 构造器中的优化默认值，性能整改必须同时验证最终绑定配置，不能只测直接 `new` 出来的单元对象。真实样本预算应按 `finishReason` 调整：关闭隐藏推理后，正文分块实际完成在约 400–500 completion token，复杂分块可给 900 的余量；全局画像 schema 更大，可独立给 1600，而不是把所有任务统一放大到数千 token。
