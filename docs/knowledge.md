...
- 导入 PDF 时文件名只作为文件展示，不作为论文标题；标题优先取 DOI/arXiv 元数据，其次从 PDF 首屏排版提取，最后要求用户手动填写。导入前文件夹推荐同时传递标题和摘要，并把文件夹展开为完整层级路径；内容明确命中某个子文件夹时优先做轻量层级匹配，否则使用向量召回和 Agent（跳过额外的 LLM 重排序），优先选择与内容最相关的最深层子文件夹。
