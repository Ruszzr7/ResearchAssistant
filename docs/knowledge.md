...
- PDF 标注的视觉类型和产品语义必须一致：`NOTE` 只能是带 `anchorQuads/anchorText` 的选区笔记，`COMMENT` 只能是带固定 `anchorPoint` 的页面批注；二者的 `notePosition` 可拖动但不能改写内容锚点。批注完成状态只适用于 `COMMENT`，由独立字段持久化并以绿色呈现。写作助手只聚合 `NOTE`，不把页面批注当成写作素材。旧的独立 `note / paper_note_link` 双表会制造第二套笔记真源，应由后续 Flyway 删除而不是与 `paper_annotation` 长期并存。
- 高亮和下划线的范围编辑属于标注本身，不应依赖全局“调整模式”：创建或点击文字标记后直接显示首尾拖柄和删除 ×；拖动只更新首/末 quad，删除按论文归属校验后立即持久化。这样 SVG 只在实际标记及控件上接管指针事件，其余文字层继续保持默认选择。
...
- 选区笔记统一复用 Annotation `NOTE`：`anchorQuads/anchorText/anchorKind=SELECTION` 绑定原文，`notePosition` 只驱动可拖动的 N 形图标和虚线，不改变原文锚点。页面批注使用 `COMMENT + anchorPoint` 与对话气泡图标；两者不能再共用“便签”语义。
...
- PDF.js 文本层上方的 SVG 标注层不能默认接收鼠标事件，否则会遮挡文本选择；默认文字选择时父层保持 `pointer-events: none`，仅让已有标记、拖柄及笔记/批注图标接管各自的指针事件，公式或页面批注落点模式才临时接管空白区域。
...
- PDF 阅读器不需要显式“选择”按钮：除公式框选和页面批注落点外，鼠标默认就是文字选择。高亮/下划线是对当前选区的显式应用；选区笔记保存 `anchorQuads + anchorText + notePosition`，页面批注保存 `anchorPoint + notePosition`，两者坐标归一化，因此缩放只需重渲染而不改变内容锚点。
...
- 阅读标注应以单条持久化对象为交互单位：创建高亮、下划线、选区笔记或页面批注时立即 POST，拖动/编辑/完成时立即 PUT，删除时立即 DELETE，不能依赖页面级“保存全部”。普通选择模式保持 SVG 空白区域不拦截文本，只有实际图标、标记和拖柄可交互。
