...
- PDF 论文分析工作台的四个顶层任务是选区问答、全文分析、跨论文对比和论文改进空间；领域研究空白不是并列入口，只能从已完成的跨论文对比结果继续。客户端根据模式构造 `intent + scope + paperIds + SelectionAnchor`，不暴露 Skill 列表；运行结果以持久化 workbench trace 为真源，任务 API 只负责阶段与终态。
