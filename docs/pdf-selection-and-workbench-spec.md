...
状态：P0 已完成；P1 的前端版面索引与同页同栏受控选取已完成（2026-07-15）。后端 `PaperLayoutArtifact`、证据链和工作台 Agent 尚未开始，本文件定义其后续建设路线。
...
| P1-B（下一步） | 后端 `PaperLayoutArtifact`、正文角色识别、`SelectionAnchor`、局部证据检索 | 真实双栏的正文顺序可持久化、可回链；页眉/页脚/参考文献不进入 evidence |
