...
### 8.18 重设计阶段 5 PDF 结构事实与论文记忆底座

默认解析链继续完全本地运行：PDFBox 生成版本化 `PaperLayoutArtifact`，保存原文 block、归一化坐标、角色、内容模式、页码和阅读顺序；现有文本选择与公式区域识别继续复用该制品。GROBID 和云解析均不进入默认依赖，前者仅作为未来可选适配器，后者当前不接入，避免部署负担和按量费用。

在版面事实之上新增确定性的 `paper-structure-v1`。它依据稳定 block ID 组织元数据、章节层级与路径、全篇阅读顺序、公式/图/表/图注元素、跨页续接边和质量指标；结构 JSON 不复制 block 全文，避免出现两个互相漂移的正文来源。`paper_memory` 以 `paperId + documentHash + layoutParserVersion + schemaVersion` 唯一版本键持久化结构，并预留后续分块摘要、全局画像、处理状态、错误码和修订号。PDF 或解析/schema 版本变化时自然重建；损坏缓存也会确定性重建。

当前全文分析入口在调用 LLM 前先执行结构门禁，保证“解析与版面识别 → 结构事实”已经完成；门禁失败时停止分析，不能在错误内容上继续花费模型 token。LLM 分块理解、全局画像、加载状态和提问上下文仍分别在阶段 6、7 实现。
...
| 重设计 5（已完成） | 本地 PDF 结构事实、版本化 `paper_memory` 与分析前置门禁 | 结构引用稳定 block；版本变化/损坏缓存可重建；结构失败不调用 LLM；默认部署不依赖 GROBID/云解析 |
