...
状态：P0 至 P3-B2 已完成并通过最终全链路审计（2026-07-16）；P4-A、P4-B 已完成并通过用户体验确认，P4-C 已提交为 `446ac36`，P4-D 与研究任务语义拆分已完成并纳入稳定提交。P4-E 已完成实现与全量/真实链路验证，待用户体验确认；P4-F 待按小步增量实施。P0–P3 的验收证据与复现命令见 [PDF 工作台最终验收](pdf-workbench-acceptance.md)。
...
| P4-E（已完成实现，待确认） | 独立翻译服务，默认 DeepL；中/英双语输出、懒翻译与缓存 | 选区翻译不调用问答 LLM；中英切换不重新分析；LaTeX/引用/数值保持；密钥不进入源码、日志或 Git |
...
实现验收（2026-07-17）：后端全量 424 项（5 项可选真实样本跳过）、前端 62 项与生产构建通过；真实 DeepL Free 请求成功，二次请求命中缓存，LaTeX、引用和数值保持。浏览器从历史英文全文分析切换中文时只调用翻译接口，原 run、claims 的 evidenceIds 与证据跳转按钮保持不变。
