...
状态：P0 至 P3-B2 已完成并通过最终全链路审计（2026-07-16）；P4-A、P4-B 已完成并通过用户体验确认，P4-C 已完成实现与自动化回归、待用户体验确认。P4-D 至 P4-F 待按小步增量实施。P0–P3 的验收证据与复现命令见 [PDF 工作台最终验收](pdf-workbench-acceptance.md)。
...
顶层导航将重复的“论文分析”和“研究空白”合并为“论文研究”；`/workbench` 复用已缓存的 `LibraryView + PdfViewer + PaperWorkbenchPanel`，而不是新建另一套 PDF/任务状态。单篇全文分析、多篇对比与研究 Gap 在同一右侧窗口按模式切换，URL 同步 `mode/paperId/paperIds`，旧 `/analysis`、`/gap` 地址保留查询参数后跳转到统一入口。任务中心、阅读计划和写作助手仍为独立页面，旧 `AnalysisView/GapView` 已删除。

后端新增第五条固定 `RESEARCH_GAP` Workflow：只接受 3–8 篇论文，逐篇读取版本绑定的本地版面 evidence，调用白名单 `IDENTIFY_RESEARCH_GAPS` Skill，并复用逐论文 Evidence Gate，要求每篇论文至少被 claim 实际引用。输出只能称为“候选空白”，必须给出跨论文证据边界、可检验问题和外部检索/实验验证步骤，不能把当前证据集未覆盖等同于领域不存在。前端 55 项、后端 407 项（5 项可选本机样本跳过）及 Vite 生产构建通过。真实浏览器验证 `/analysis?mode=compare` 与 `/gap` 均保留参数跳转到统一模式；当前库只有一篇带 PDF 论文时 Gap 显示三篇门槛并禁用执行，切到任务中心后返回仍恢复 Gap 工作区，控制台无 error。
...
| P4-C（已完成，待用户确认） | 统一单篇分析、研究 Gap 与多篇对比的页面、路由和 Workflow 入口 | 同一窗口按范围切换任务；旧入口兼容跳转；任务中心、阅读计划、写作助手保持独立 |
